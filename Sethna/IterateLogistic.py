...
def BifurcationDiagram(g, x0, nTransient, nCycle, etaArray, showPlot=True):
    ...
    xMatrix = []
    for eta in etaArray:
        etaMatrix.append([eta]*nCycle)
        xMatrix.append(IterateList(g, Iterate(g, x0, nTransient, (eta,)),
                              nCycle, (eta,)))
    pylab.plot(etaMatrix, xMatrix, 'k,')
...
def demo():
    ...
    BifurcationDiagram(f, 0.1, 500, 500, scipy.arange(0., 1.00001, 0.005))
    print "  Creating Bifurcation Diagram (zoom)"
    BifurcationDiagram(f, 0.1, 500, 500, scipy.arange(0.85, 1.00001, 0.0005))
    print "  Creating Bifurcation Diagram (zoom again)"
    BifurcationDiagram(f, 0.1, 500, 500, scipy.arange(0.96, 0.97, 0.000025))
